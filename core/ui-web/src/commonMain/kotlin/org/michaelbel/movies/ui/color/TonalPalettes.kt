...
private typealias TonalPalette = Map<Double, Color>
...
class TonalPalettes(
    ...
    infix fun accent1(tone: Double): Color {
        return accent1.getOrElse(tone) { keyColor.transform(tone, style.accent1Spec) }
    }

    infix fun accent2(tone: Double): Color {
        return accent2.getOrElse(tone) { keyColor.transform(tone, style.accent2Spec) }
    }

    infix fun accent3(tone: Double): Color {
        return accent3.getOrElse(tone) { keyColor.transform(tone, style.accent3Spec) }
    }

    infix fun neutral1(tone: Double): Color {
        return neutral1.getOrElse(tone) { keyColor.transform(tone, style.neutral1Spec) }
    }

    infix fun neutral2(tone: Double): Color {
        return neutral2.getOrElse(tone) { keyColor.transform(tone, style.neutral2Spec) }
    }
