...
class SettingsRepositoryImpl(
    ...
    override val currentTheme: Flow<AppTheme>
        get() = preferences.getValueFlow(MoviesPreferences.PreferenceKey.PreferenceThemeKey)
            .map { name -> AppTheme.transform(name ?: AppTheme.FollowSystem.toString()) }

    override val currentFeedView: Flow<FeedView>
        get() = preferences.getValueFlow(MoviesPreferences.PreferenceKey.PreferenceFeedViewKey)
            .map { name -> FeedView.transform(name ?: FeedView.FeedList.toString()) }

    override val currentMovieList: Flow<MovieList>
        get() = preferences.getValueFlow(MoviesPreferences.PreferenceKey.PreferenceMovieListKey)
    ...
    override val isBiometricEnabled: Flow<Boolean>
        get() = preferences.getValueFlow(MoviesPreferences.PreferenceKey.PreferenceBiometricKey).map(Boolean?::orEmpty)

    override val isScreenshotBlockEnabled: Flow<Boolean>
        get() = preferences.getValueFlow(MoviesPreferences.PreferenceKey.PreferenceScreenshotBlockKey).map(Boolean?::orEmpty)
